...
export default function SchedulePage() {
  ...
  // Calculate positions for overlapping schedule items
  const calculateItemPositions = (items: ScheduleItem[]) => {
    // Group items by day
    const itemsByDay: Record<string, ScheduleItem[]> = {};
    items.forEach((item) => {
    ...
    // For each day, calculate proper stacking based on start time
    const positionedItems = items.map((item) => ({ ...item, stackIndex: 0, totalStack: 1 }));
    ...
    Object.keys(itemsByDay).forEach((day) => {
      ...
      // Sort by start time (earliest first for bottom position)
      dayItems.sort((a, b) => a.startSlot - b.startSlot);

      // For each item, count how many earlier items it overlaps with
      dayItems.forEach((item, index) => {
        let overlapCount = 0;

        // Check overlap with all previous items
        for (let i = 0; i < index; i++) {
          const prevItem = dayItems[i];
          const itemEnd = item.startSlot + item.duration;
          const prevEnd = prevItem.startSlot + prevItem.duration;

          // Two items overlap if one starts before the other ends
          if (item.startSlot < prevEnd && itemEnd > prevItem.startSlot) {
            overlapCount++;
          }
        }

        // Stack index = number of earlier overlapping items (earliest at bottom)
        const positionedItem = positionedItems.find((pi) => pi.id === item.id);
        if (positionedItem) {
          positionedItem.stackIndex = overlapCount;
          positionedItem.totalStack = dayItems.length;
        }
      });
    });
  ...
  const positionedItems = calculateItemPositions(visibleItems);
            ...
            >
              ...
              <div className="absolute inset-0 pointer-events-none z-30" style={{ minWidth: '60px' }}>
                {positionedItems.map((item) => {
                  ...
                  const stackOffset = item.stackIndex * 15;
                  const adjustedTopPos = topPos - stackOffset;

                  // Calculate width based on total items in stack
                  const widthOffset = item.totalStack > 1 ? 10 : 0;
                  const adjustedWidth = dayWidth - 10 - widthOffset;
                  ...
                  // Z-index: 100-115 for normal, 9999 when hovered (always above sticky elements)
                  ...
                  const hoverShadow = isHovered ? '0 10px 25px -5px rgba(0, 0, 0, 0.2)' : '';

                  return (
                    <div
                      key={item.id}
                      className={`absolute p-1 rounded-md text-xs cursor-pointer transition-all duration-200 pointer-events-auto select-none ${
                        isLecture
                          ? 'bg-indigo-50 border-l-4 border-indigo-600'
                          : 'bg-pink-50 border-l-4 border-pink-600'
                      } ${user?.role === 'professor' ? 'opacity-70' : ''}`}
                      style={{
                        top: `${adjustedTopPos + hoverTranslateY}px`,
                        height: `${heightPos}px`,
                        left: `${leftPx}px`,
